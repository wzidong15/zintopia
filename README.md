...
1. Pick a **preset**: the four ★ entries are the best-tested combinations on 2017–2026 US ETF data (SPY above SMA 150; accelerated dual momentum SPY / EFA with SHY; 12-month sector rotation top 2; the RSI(2) < 15 overlay), plus Double 7s, Bollinger, Turtle breakouts, the Faber and dual-momentum grids, sector rotation, and an SMA grid. Alternatively load a **paper fund's strategy**, or choose a strategy and type your own parameter lists (`10, 20, 50`). Every list-valued parameter is expanded into a grid.
2. Set symbols (max 12), start / end, initial cash, commission and slippage in basis points, and the ranking key.
3. **Run grid** ranks every combination of the chosen strategy. **Compare all rules** runs each strategy at its defaults on the same symbols so you can see which family suits them before tuning.
4. Results open with a **verdict card**: in-sample and out-of-sample CAGR against buy & hold, max drawdown, Sharpe, trades and win rate, walk-forward efficiency, and two plain sentences saying what the numbers mean. Below the chart, tabs hold the ranking, the walk-forward folds, the parameter search, the last trades, and the notes. Click a ranking row to chart it; the top 12 runs keep an equity curve. Recent runs stay as chips so you can flip between them. Costs, ranking key, walk-forward settings, and the search budget sit under **Costs & validation**.
5. **Optimize parameters**: random sampling over the strategy's search ranges (shown under each box), then coordinate hill-climbing from the leaders, up to the chosen budget (100–800 evaluations; grids smaller than the budget run exhaustively). A box with one value pins that parameter, several values become the axis, blank uses the default range. The search table shows the top 10 with the mean objective of their ±1-step neighbours and a **plateau / spike** verdict, and sensitivity bars show how the objective moves as each parameter varies with the others held at the winner. Every sampled point also appears in the ranking and feeds walk-forward, so the OOS block is the honest read on the search.
...
| Double 7s | Connors' ETF rule: buy when the close is the lowest close of the last N days (7 in the book) and above the trend SMA; sell when it is the highest close of the last N days. |
| Bollinger mean reversion | Buy when the close drops below SMA − k·σ; sell above the middle band (exit k = 0) or the upper band. Optional trend SMA. |
| Donchian breakout | Turtle: buy when the close exceeds the highest close of the prior N days; sell below the lowest close of the prior M days. 20/10 and 55/20 in the original systems. |
...
| `POST /api/backtest/optimize` | Parameter search: `{symbols, start, end, strategy, space?:{key:[values] or {min,max,step}}, fixed?:{key:value}, objective, budget, seed?, walk_forward?}` → same shape as `POST /api/backtest` plus a `search` block (top 10 with neighbourhood stability, sensitivity slices, phases) |
